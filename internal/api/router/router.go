...
func New(cfg *config.Config, log *zap.Logger) (
	...
	*service.ScreenerTemplateService,
	...
	riskSvc := service.NewRiskService(riskRepo, tradeRepo, tradeSvc, stockRepo, stockReportRepo, log)
	...
	scoreHandler := handler.NewStockScoreHandler(scoreSvc, log)
	...
	v1 := r.Group("/api/v1")
	{
		...
		v1.GET("/morning-brief", morningBriefHandler.Get)
